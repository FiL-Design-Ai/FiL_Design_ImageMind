...
| 17 | FiLEditEncoder | promoted | новая нода (после 1.1.2): edit-кондей «всё-в-одном» для семейства FLUX.2 (Krea2/Klein/Dev), см. секцию ниже |
...
## 17. FiLEditEncoder — promoted (новая нода, добавлена после 1.1.2)

- **Статус:** promoted (в `RELEASE_NODES`) — узел добавляется сразу с чек-листом и тестами.
- **Задача:** edit-кондей «всё-в-одном» для моделей семейства FLUX.2 (Krea2, Klein, Dev).
  Ядро ComfyUI собирает такой пайплайн из разрозненных нод — `ImageScaleToTotalPixels` →
  `VAEEncode` → цепочка `ReferenceLatent` (по одной на картинку) + отдельный
  `CLIPTextEncode`. Нода сворачивает цепочку в один узел: промпт + авто-растущие входы
  референсов (до 10) → текст-энкод + VAE-кодирование каждого референса до бюджета
  мегапикселей → `reference_latents` и `reference_latents_method` в кондее.
- **Файлы:** `nodes/node_edit_encoder.py`, контракт `common/contracts/nodes/edit_encoder.py`,
  фронтенд `frontend/src/nodes2/nodes/edit_encoder.ts` + панель
  `components/nodes/EditEncoderPanel.vue` (textarea промпта, слайдер мегапикселей,
  селект метода и живой счётчик подключённых референсов; Autogrow-слоты остаются
  под панелью), тесты `tests/test_edit_encoder.py`, локали `ee_*`/`eep_*`,
  новая категория `CATEGORY_CONDITIONING` в `common/brand.py`, литерал `CONDITIONING`
  в `common/contracts/schema.py`.
- **Решения по архитектуре:**
  - Модельно-агностичная: нода ничего не знает о конкретной edit-модели — только ресайз,
    VAE-энкод и передача `reference_latents` тем же способом, что и корневой
    `ReferenceLatent`. Подходит любой модели, читающей референсы по-FLUX.2-овски.
  - Порядок референсов фиксируется по числовому суффиксу слота, а не по порядку словаря —
    Autogrow отдаёт dict, и кондей должен совпадать с порядком проводов.
  - Вес отдельных референсов сознательно не делаем: ядро конкатит латенты без весов
    (`comfy/model_base.py`, `QwenImage`/`Flux`-ветки), реализация потребовала бы форка
    ldm-модели. Маски тоже не здесь — они живут на уровне латентов.
  - Fail Fast: референсы без подключённого VAE — исключение с понятным сообщением,
    а не тихий текст-энкод.
  - `auto_size` (default ON): авторежим мегапикселей — референсы меньше бюджета
    остаются в родном размере (без апскейла: меньше латент-токенов, нет выдуманных
    деталей), бюджет работает потолком; OFF — точный ресайз в бюджет, как
    `ImageScaleToTotalPixels`. На панели слайдер в auto меняет подпись на
    «потолок мегапикселей».
- **Находка (закрыта):**
  - [x] Первый живой прогон из UI упал на парсере порядка слотов: фронт зовёт
    Autogrow-слоты `image1`/`image2` (без подчёркивания), а парсер ждал API-написание
    `image_1` и умер на `int('image1')`. Теперь порядок читается по хвостовым цифрам
    имени и переживает оба написания; тест `test_ui_style_slot_names_keep_the_wired_order`
    фиксирует именно упавший случай.
- **Тесты:** `tests/test_edit_encoder.py` — схема, пустой граф (чистый текст-кондей),
  ресайз до бюджета с выравниванием /8 и сохранением пропорций, обрезка альфы, порядок
  слотов при перемешанном словаре, проброс метода, fail-fast без VAE. Контрактный паритет,
  локализация и wiring покрыты общими тестами.
