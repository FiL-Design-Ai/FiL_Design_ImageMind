/**
 * The pre-split `domWidgetHost.ts`, exactly as it stood at dc873c0.
 *
 * Kept only so `stretchDrift.test.ts` can run the same sequence through both
 * implementations and compare. Never imported by `src/`. The two exported names
 * carry a `Legacy` suffix so both versions can be imported side by side; that
 * is the whole of the difference. If it ever needs editing to keep compiling,
 * the comparison it exists for is already void — delete it instead.
 */
/**
 ...
 */
export function addLegacyFilDomWidget<S extends object = Record<string, unknown>>(
  node: unknown,
  name: string,
  root: Component,
...
export function unmountAllFilWidgetsLegacy(node: unknown): void {
